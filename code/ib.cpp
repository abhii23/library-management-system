...
void Lib::issue()
{
    ...
    system("cls");
    cout<<"\n\t\t->Please Choose one option :-\n";
    cout<<"\n\t\t1.Issue Book\n\n\t\t2.View Issued Book\n\n\t\t3.Search student who isuued books\n\n\t\t4.Reissue Book\n\n\t\t5.Return Book\n\n\t\t6.Go back to menu\n\n\n\t\t \n\t\t NOTE- AFTER 30 DAYS ,THE FINE WILL CHARGE $2PER DAY \n\n\t\tEnter Your Choice : ";
    cin>>i;
    fflush(stdin);
    if(i==1)
    {
...
{
    const int fineRatePerDay = 2; 

    // Calculate the number of days late
    int daysLate = (dd - d) + (mm - m) * 30 + (yy - y) * 365;
    int day=daysLate-30;

    // Calculate the fine amount
    int fineAmount = (day > 0) ? day * fineRatePerDay : 0;

    // Output the fine amount if applicable
    if (fineAmount > 0)
        cout << "\n\t\tThe total fine amount is: " << fineAmount << endl;
